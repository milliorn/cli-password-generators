...
After the build succeeds, a binary named `password-generator` will be present in the `golang/` directory. You can move it anywhere on your `$PATH` if you want system-wide access.
...
The TypeScript source must be compiled to plain JavaScript before execution. The `dist/` directory is not committed to the repository — you must build it locally after installing dependencies.
...
└── node/
    ...
    ├── passwords.txt          ← Generated at runtime when --save is used (gitignored)
    ├── readme.md              ← Node-specific notes
    ├── dist/                  ← Compiled JS output — generated by npm run rebuild (gitignored)
    └── src/
